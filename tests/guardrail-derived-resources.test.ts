...
describe("graph-derived resource grants", () => {
  ...
  it("derives a filesystem grant from a node:fs import", () => {
  ...
  it("derives the same grants when the scope is the containing file", () => {
  ...
  it("derives network and subprocess grants from builtin imports", () => {
  ...
  it("does not derive anything for scopes without resource-touching imports", () => {
  ...
  it("stays fail-closed for requires hidden inside function bodies", () => {
  ...
  it("does not derive grants for builtins that map to no resource category", () => {
    const allowlist = generateAllowlist("fn:py/osuser.py#home_dir", store);
    expect(allowlist.derivedResources).toEqual([]);
  });

  it("merges config grants additively with derived ones", () => {
    const allowlist = generateAllowlist("fn:src/storage.js#loadSettings", store, {
      resources: {
        ...emptyResourceGrants(),
  ...
  it("derives nothing from non-builtin external imports in the original fixture repo", () => {
  ...
  it("lets derived grants through the interceptor gate and keeps others closed", () => {
    const storage = generateAllowlist("fn:src/storage.js#loadSettings", store);
    const allowed = checkAndRun("require('node:fs')", storage);
    expect(allowed.outcome).toBe("executed");

    const sneaky = generateAllowlist("fn:src/sneaky.js#sneaky", store);
    const blocked = checkAndRun("require('dns')", sneaky);
